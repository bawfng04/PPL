...
class CodeGenerator(BaseVisitor,Utils):
    ...
    def visitFieldAccess(self, ast: FieldAccess, o: dict) -> tuple[str, Type]:
        ...
        # If type is ClassType or Id, look up in list_type
        if isinstance(typ, ClassType) and typ.name in self.list_type:
            typ = self.list_type[typ.name]

        # Find field in struct elements
        field_type = None
        for field_name, field_type in typ.elements:
            if field_name == ast.field:
                break

        if not field_type:
            return "", VoidType()  # Field not found

        # Generate code based on whether this is an lvalue or rvalue
        isLeft = o.get("isLeft", False)

        if isLeft:
            return (
                code
                + self.emit.emitPUTFIELD(f"{typ.name}/{ast.field}", field_type, o["frame"]),
                field_type,
            )
        else:
            return (
                code
                + self.emit.emitGETFIELD(f"{typ.name}/{ast.field}", field_type, o["frame"]),
                field_type,
            )
    ...
    def visitMethCall(self, ast: MethCall, o: dict) -> tuple[str, Type]:
        ...
        # Look up the type details if it's a type name
        if isinstance(typ, ClassType) and typ.name in self.list_type:
            typ = self.list_type[typ.name]
        ...
        for arg in ast.args:
            ...
            argCodes.append(argCode)

        # Combine receiver code and argument codes
        code += "".join(argCodes)
        ...
        # Handle statement vs expression context
        if isStmt:
    ...
    def visitStructLiteral(self, ast: StructLiteral, o: dict) -> tuple[str, Type]:
        ...
        for item in ast.elements:
            fieldCode, fieldType = self.visit(item[1], o)
            code += fieldCode

        # Call the constructor
        code += self.emit.emitINVOKESPECIAL(
            o["frame"], f"{ast.name}/<init>", MType([], VoidType())
        )

        # Return the generated code and the struct type
